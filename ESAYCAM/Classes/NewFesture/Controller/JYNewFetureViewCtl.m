...
//    int startPoint = (maxW/2) - (pageCtlWidth/2);
    int startPoint = (maxW - pageCtlWidth) * 0.5;
...
//        int introImgViewY = item.title ? titleLabel. y- 10 - 50 : descLabel.y - 10 - 50;
        ...
        // 文字上面的图片
        UIImageView *introImgView = [[UIImageView alloc] initWithImage:[UIImage imageWithImage:[UIImage imageNamed:item.img] scaledToWidth:screenW]];
        introImgView.frame = CGRectMake((screenW - introImgView.size.width) * 0.5, self.pageView.y - 5 - introImgView.size.height, introImgView.size.width, introImgView.size.height);
//        introImgView.backgroundColor = [UIColor yellowColor];
