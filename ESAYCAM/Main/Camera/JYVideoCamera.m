...
- (void)takePhotoWithArray
{
    [self.videoCamera capturePhotoAsImageProcessedUpToFilter:self.filter withOrientation:UIImageOrientationUp withCompletionHandler:^(UIImage *processedImage, NSError *error) {
        ...
        switch ((NSInteger)self.isLongExposure) {
            case 0:
            {
                UIImageWriteToSavedPhotosAlbum(processedImage, nil, nil, nil);
                if (self.delegate && [self.delegate respondsToSelector:@selector(cameraManageTakingPhotoSucuess:)]) {
            ...
            case 1:
            {
                ...
                if (self.imgsArray.count == self.arrCount - 1) {
                    if (self.delegate && [self.delegate respondsToSelector:@selector(videoCameraWillCompera)]) {
                        [self.delegate videoCameraWillCompera];
                    }
                }
                if (self.imgsArray.count >= self.arrCount) {
                    if (self.delegate && [self.delegate respondsToSelector:@selector(videoCameraCompera)]) {
                        [self.delegate videoCameraCompera];
                    }
                }
            }
                break;
...
- (GPUImageStillCamera *)videoCamera
{
    if (!_videoCamera) {
        ...
        self.videoConnection = [_videoCamera videoCaptureConnection];
